"""
Authorization Server for Notebook MCP Server.

This server handles OAuth flows, client registration, and token issuance
with support for multiple scopes ('read' and 'write').
"""
...
from .notebook_provider import NotebookAuthSettings, NotebookOAuthProvider
...
class NotebookAuthProvider(NotebookOAuthProvider):
    """
    Authorization Server provider with simple demo authentication.

    This provider:
    1. Issues MCP tokens after simple credential authentication
    2. Supports multiple scopes ('read' and 'write')
    3. Stores token state for introspection by Resource Servers
    """

    def __init__(self, auth_settings: NotebookAuthSettings, auth_callback_path: str, server_url: str):
        super().__init__(auth_settings, auth_callback_path, server_url)


def create_authorization_server(server_settings: AuthServerSettings, auth_settings: NotebookAuthSettings) -> Starlette:
    """Create the Authorization Server application."""
    oauth_provider = NotebookAuthProvider(
    ...
    mcp_auth_settings = AuthSettings(
        ...
        client_registration_options=ClientRegistrationOptions(
            enabled=True,
            valid_scopes=["read", "write"],  # Support both read and write scopes
            default_scopes=["read"],  # Default to read scope
        ),
        required_scopes=None,  # No required scopes at server level (tools define their own)
...
async def run_server(server_settings: AuthServerSettings, auth_settings: NotebookAuthSettings):
    ...
    logger.info(f"🚀 Notebook MCP Authorization Server running on {server_settings.server_url}")
    logger.info(f"📝 Supported scopes: read, write")
...
def main(port: int) -> int:
    """
    Run the Notebook MCP Authorization Server.

    This server handles OAuth flows and can be used by the Notebook Resource Server.
    ...
    # Load auth settings
    auth_settings = NotebookAuthSettings()
